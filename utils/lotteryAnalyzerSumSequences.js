// [SỬA ĐỔI QUAN TRỌNG] - Thêm đối tượng này vào đầu file
const sumLists = {
    traditional: {
        ...
        10: [0, 19, 28, 37, 46, 55, 64, 73, 82, 91] // Gộp cả 00
    },
    new: {
        0: [0], 1: [1, 10], 2: [2, 11, 20], 3: [3, 12, 21, 30],
        4: [4, 13, 22, 31, 40], 5: [5, 14, 23, 32, 41, 50],
        6: [6, 15, 24, 33, 42, 51, 60], 7: [7, 16, 25, 34, 43, 52, 61, 70],
        8: [8, 17, 26, 35, 44, 53, 62, 71, 80], 9: [9, 18, 27, 36, 45, 54, 63, 72, 81, 90],
        10: [19, 28, 37, 46, 55, 64, 73, 82, 91], 11: [29, 38, 47, 56, 65, 74, 83, 92],
        12: [39, 48, 57, 66, 75, 84, 93], 13: [49, 58, 67, 76, 85, 94],
        14: [59, 68, 77, 86, 95], 15: [69, 78, 87, 96],
        16: [79, 88, 97], 17: [89, 98], 18: [99]
    }
};
...
const findAdvancedSumSequences = (data, options) => {
    ...
    const calculateSum = (num, currentSumType) => {
        ...
        const tail = num % 10;
        const rawSum = head + tail;
        if (currentSumType === 'traditional') {
             if (num === 0) return 10;
             const modSum = rawSum % 10;
             return modSum === 0 ? 10 : modSum;
        }
        return rawSum;
    };

    const getSumParityPattern = (sum) => {
        if (sum === null || sum < 0 || sum > 18) return null;
        const head = Math.floor(sum / 10);
        const tail = sum % 10;
        return `${head % 2 === 0 ? 'even' : 'odd'}-${tail % 2 === 0 ? 'even' : 'odd'}`;
    };

    const createResultObject = (sequence, dayDetails, seqType = 'sum') => {
        return {
            sequence: sequence,
            ...
            results: dayDetails.map((day, idx) => {
                ...
                if (seqType === 'sum') {
                    extractedNumbers = originalNumbers.filter(n => calculateSum(n, sumType) === matchedValue);
                } else { 
                    extractedNumbers = [matchedValue];
                }
                return {
                    date: day.date,
    ...
    for (let i = 0; i <= filteredData.length - consecutiveDays; i++) {
        ...
        if (analysisType.startsWith('common_sum')) {
            const minSum = sumType === 'traditional' ? 1 : 0;
            const maxSum = sumType === 'traditional' ? 10 : 18;

            for (let sum = minSum; sum <= maxSum; sum++) {
                const numbersWithSumPerDay = days.map(day => 
                    [...new Set(getNumbersByMode(day, mode).filter(num => calculateSum(num, sumType) === sum))].sort((a, b) => a - b)
                );
                if (numbersWithSumPerDay.some(arr => arr.length === 0)) continue;

                // [SỬA ĐỔI QUAN TRỌNG] - Logic mới cho "Dần Đều" dựa trên vị trí trong danh sách
                if (analysisType.includes('arithmetic')) {
                    const sumNumberList = sumLists[sumType][sum];
                    if (!sumNumberList) continue;

                    let sequences = numbersWithSumPerDay[0].map(val => [val]);
                    for (let dayIndex = 1; dayIndex < consecutiveDays; dayIndex++) {
                        const newSequences = [];
                        for (const seq of sequences) {
                            const lastValue = seq[seq.length - 1];
                            const lastValueIndex = sumNumberList.indexOf(lastValue);
                            if (lastValueIndex === -1) continue;

                            for (const nextValue of numbersWithSumPerDay[dayIndex]) {
                                const nextValueIndex = sumNumberList.indexOf(nextValue);
                                if (nextValueIndex === -1) continue;

                                let condition = false;
                                if (analysisType.includes('increasing') && nextValueIndex === lastValueIndex + 1) {
                                    condition = true;
                                } else if (analysisType.includes('decreasing') && nextValueIndex === lastValueIndex - 1) {
                                    condition = true;
                                }
                    ...
                    sequences.forEach(seq => results.push(createResultObject(seq, dayDetails, 'number')));
                } 
                else if (analysisType === 'common_sum_occurrence') {
                    const representativeSequence = numbersWithSumPerDay.map(dayNumbers => dayNumbers[0]);
                    results.push(createResultObject(representativeSequence, dayDetails, 'number'));
                } 
                else { // Logic cho tăng/giảm không đều (giữ nguyên)
                    let sequences = numbersWithSumPerDay[0].map(val => [val]);
                    for (let dayIndex = 1; dayIndex < consecutiveDays; dayIndex++) {
                        const newSequences = [];
                        for (const seq of sequences) {
                            ...
                            for (const nextValue of numbersWithSumPerDay[dayIndex]) {
                                let condition = false;
                                if (analysisType === 'common_sum_increasing') condition = nextValue > lastValue;
                                else if (analysisType === 'common_sum_decreasing') condition = nextValue < lastValue;
        ...
        else { // Logic cho chuỗi TỔNG (giữ nguyên, đã đúng)
            const dayDetailsWithSums = days.map(day => {
                const values = new Set();
                getNumbersByMode(day, mode).forEach(num => {
                    const value = calculateSum(num, sumType);
                    let isValid = false;
                    if (analysisType === 'sum') {
                        if (sumParity === 'any' || !sumParity) isValid = true;
                        else if (sumParity === 'even' && value % 2 === 0) isValid = true;
                        else if (sumParity === 'odd' && value % 2 === 1) isValid = true;
                    } else if (analysisType === 'sum_parity_pattern') {
                        if (getSumParityPattern(value) === numberParity) isValid = true;
                    }
                ...
                return {
                    date: day.date,
                    dayData: day,
                    validSumsList: [...values].sort((a,b) => a-b)
                };
            });

            if (dayDetailsWithSums.some(detail => detail.validSumsList.length === 0)) continue;
            
            if (pattern === 'consecutive_occurrence') {
                if (sumParity === 'any' || !sumParity) {
                    const firstDaySums = dayDetailsWithSums[0].validSumsList;
                    for (const sum of firstDaySums) {
                        if (dayDetailsWithSums.every(detail => detail.validSumsList.includes(sum))) {
                            const sequence = Array(consecutiveDays).fill(sum);
                            results.push(createResultObject(sequence, dayDetailsWithSums, 'sum'));
                        }
                    }
                } else {
                    const representativeSequence = dayDetailsWithSums.map(detail => detail.validSumsList[0]);
                    results.push(createResultObject(representativeSequence, dayDetailsWithSums, 'sum'));
                }
            } else {
                let sequences = dayDetailsWithSums[0].validSumsList.map(val => [val]);
                for (let dayIndex = 1; dayIndex < consecutiveDays; dayIndex++) {
                    const newSequences = [];
                    for (const seq of sequences) {
                        const lastValue = seq[seq.length - 1];
                        for (const nextValue of dayDetailsWithSums[dayIndex].validSumsList) {
                            let condition = false;
                            if (pattern.includes('arithmetic')) {
                                const isParitySpecific = (sumParity && sumParity !== 'any') || analysisType === 'sum_parity_pattern';
                                const diff = isParitySpecific ? 2 : 1;
                                if (pattern === 'arithmetic_increasing') {
                                    condition = (nextValue === lastValue + diff);
                                } else if (pattern === 'arithmetic_decreasing') {
                                    condition = (nextValue === lastValue - diff);
                                }
                            } else {
                                if (pattern === 'monotonic_increasing') {
                                    condition = nextValue > lastValue;
                                } else if (pattern === 'monotonic_decreasing') {
                                    condition = nextValue < lastValue;
                                }
                            }
                            if (condition) newSequences.push([...seq, nextValue]);
                ...
                sequences.forEach(seq => results.push(createResultObject(seq, dayDetailsWithSums, 'sum')));
            }
        }
    }
    ...
    return { 
        results: uniqueResults.slice(0, 200), 
        total, 
